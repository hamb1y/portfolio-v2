...
# 2. How it worked
The system had three main parts working together. The frontend was written in Svelte 5 and was the main thing participants interacted with. It handled the survey flow, audio playback, and collecting ratings. It also acted as the orchestrator for the heart sensor data, ingesting JSON from the sensor pipeline every 5 seconds. It bundled this as partial JSON updates along with the survey results, identified by a UUID, and sent them to the backend.

The backend was a lightweight Flask REST API that received these partial JSON updates and stored them as individual JSON files per session. After collection, we compressed and encrypted the results using a pipeline of eXdupe, then zstd, then age with a public/private key pair. This meant only I had access to the private key, but anyone could encrypt data to the public key, keeping the research data secure.
